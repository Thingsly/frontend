...
>(config: TableConfig<TableData, Fn, CustomColumnKey>) {
  ...
  const { columns, filteredColumns, reloadColumns } = useTableColumn(
    config.columns
  );
  ...
  async function getData() {
    ...
    const {
      ...
      pageSize,
      total
    } = transformer(response as Awaited<ReturnType<Fn>>);
...
function useTableColumn<TableData extends BaseData, CustomColumnKey = never>(
  ...
  const allColumns = ref(factory()) as Ref<
    TableColumn<TableData, CustomColumnKey>[]
  >;

  const filteredColumns: Ref<FilteredColumn[]> = ref(
    getFilteredColumns(factory())
  );
  ...
  function getFilteredColumns(
    aColumns: TableColumn<TableData, CustomColumnKey>[]
  ) {
  ...
  function getColumns() {
    const cols = filteredColumns.value
      ...
      .map((column) => {
        ...
        return allColumns.value.find(
          (col) => (col as DataTableBaseColumn).key === column.key
        );
      });
