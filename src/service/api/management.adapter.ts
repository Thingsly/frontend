...
/**
 * Process the tree structure of the route data
 *
 * @param treeNode
 * @param depth
 */
...
export function adapterOfFetchRouterList(
  data: Api.Route.Data
): Api.Route.MenuRoute[] {
  if (!data?.list) return [];
...
function replaceKeys(data: ElegantConstRoute[]): ElegantRoute[] {
  return data.map((item: any): ElegantRoute => {
    ...
    return {
      ...
      path:
        item.param1 && item.param1[0] === '/'
          ? item.param1
          : (`/${item.param1}` as string),
...
export function adapterOfFetchUserRouterList(
