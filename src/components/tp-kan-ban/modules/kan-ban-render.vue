...
function onDrop(event) {
  ...
  if (cardItem.cardItemBase.minWH.minW !== -1) {
    if (typeof cardItem.cardItemBase.minWH.minW === 'number') {
      ...
      minWh.minW = Math.ceil(
        Number(w) / (gridLayout?.value?.state?.width || 0 / KANBANCOLNUM)
      );
    }
  }
  ...
  mouseAt.x = event.clientX - rect.left;
  mouseAt.y = event.clientY - rect.top;

  const itemId =
    cardItem?.cardItemBase?.renderID ||
    `${theLayout.value.length.toString()}_${cardItem.cardItemBase.id}`;
  const item: CardView = {
...
<template>
  <n-card
    class="h-full w-full"
    content-class="h-full w-full custom-scroll"
