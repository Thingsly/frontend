...
const updateDisabledOptions = (timeFrame: string) => {
  ...
  dataAggregateRangeOptions.forEach((item, index, array) => {
    ...
    if (index < (disableBeforeIndex[timeFrame] || 0)) {
      state.data.dataSource.dataAggregateRange = array[index + 1]
        .value as string;
    }
  });
};
...
const deviceCountUpdate = (v) => {
  ...
  if (state.data.dataSource.deviceSource.length < v) {
    ...
    for (
      let i = 0;
      i <= v - state.data.dataSource.deviceSource.length + 1;
      i++
    ) {
      state.data.dataSource.deviceSource.push({});
    }
  } else if (state.data.dataSource.deviceSource.length > v) {
    state.data.dataSource.deviceSource.splice(
      v,
      state.data.dataSource.deviceSource.length - v
    );
  }
};
...
<template>
  <div>
    <NTabs
      v-if="state.selectCard"
      v-model:value="state.tab"
      type="line"
      animated
    >
      <NTabPane
        v-if="state.selectCard.type === 'chart'"
        name="dataSource"
        :tab="$t('card.dataSource')"
      >
        <div
          :class="`${mobile ? '' : 'h-[calc(100vh_-_150px)] '} overflow-y-auto py-5`"
        >
          <NForm>
            <NSelect
            ...
            <div
              v-if="
                state.data.dataSource?.origin === 'device' ||
                state.data.dataSource?.origin === 'system'
              "
            >
              <n-input-number
              ...
              >
                ...
                <NSelect
                  v-if="
                    i <= deviceCount - 1 &&
                    state.data.dataSource.isSupportAggregate
                  "
