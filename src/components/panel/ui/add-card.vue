...
const save = () => {
  ...
  if (
    state &&
    state.curCardData &&
    state.curCardData.dataSource &&
    state.curCardData.dataSource.deviceSource
  ) {
    state.curCardData.dataSource.deviceSource =
      state.curCardData.dataSource.deviceSource.filter(
        (item) => Object.keys(item).length > 0
      );
    if (
      state.curCardData.dataSource.deviceCount >= 1 &&
      state.curCardData.dataSource.deviceSource.length >
        state.curCardData.dataSource.deviceCount
    ) {
      state.curCardData.dataSource.deviceSource.splice(
        state.curCardData.dataSource.deviceCount
      );
    }
...
onUpdated(() => {
  if (
    ...
    props?.data?.dataSource?.deviceSource?.length > 0
  ) {
    deviceSelectId.value =
      props?.data?.dataSource?.deviceSource[0]?.deviceId || '';
    collectData(
      deviceSelectId.value,
      deviceOptions.value?.find(
        (item) => item.device_id === deviceSelectId.value
      )
    );
  } else {
...
<template>
  ...
  >
    <div class="h-[calc(100vh_-_170px)] w-full flex">
      <div
        :class="
          'relative h-full flex flex-col p-4 overflow-hidden  ' + widths[0]
        "
        :style="
          count === 2
      ...
      >
        ...
        >
          <NTabPane
            v-for="item1 in tabList"
            :key="item1.type"
            class="h-full"
            :name="item1.type"
            :tab="item1.tab"
          >
            <div v-if="item1.tab === 'device'">
              <NSelect
              ...
              <div v-if="item1.tab === 'device'">
                <n-grid :x-gap="10" :y-gap="10" cols="1 240:1 480:2 720:3">
                  ...
                  >
                    ...
                    >
                      <div
                        class="text-center font-medium leading-8 dark:bg-zinc-900"
                      >
                        {{ $t(item.data.title) }}
                      </div>
...
-->
              ...
              <div v-else>
                <n-grid :x-gap="10" :y-gap="10" cols="1 240:1 480:2 720:3">
                  <n-gi
                    v-for="item in PanelCards[item1.type]"
                    :key="item.id"
                    class="min-w-240px"
                  >
                    <div
        ...
        <n-float-button
          ...
          shape="square"
        >
          <span class="text-12px text-primary-600">{{
            $t('generate.expand-card')
          }}</span>
        </n-float-button>
      </div>
      <div
        :class="
          'h-full flex-center justify-center border-r bg-[#f6f9f8] p-2 overflow-hidden ' +
          widths[1]
        "
      >
      ...
      >
        ...
        >
          <span class="text-12px text-primary-600">{{
            $t('generate.expand-configuration')
          }}</span>
        </n-float-button>
      </div>
    </div>
    <div class="h-60px flex flex-center border-t">
      <div>
        ...
        <NButton class="mr-4" type="primary" @click="save">{{
          $t('generate.confirm')
        }}</NButton>
      </div>
    </div>
    <div
      v-if="count === 1"
      class="absolute bottom-0 right-0 h-60px flex flex-center"
    >
      <n-icon size="24">
