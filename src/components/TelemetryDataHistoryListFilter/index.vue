...
const allAggregateWindowOptions: Array<{
  label: string;
  ...
  {
    label: 'No aggregate',
    value: 'no_aggregate' as AggregateWindowValue,
    seconds: windowToSeconds('no_aggregate')
  },
  {
    label: '30s',
    value: '30s' as AggregateWindowValue,
    seconds: windowToSeconds('30s')
  },
  {
    label: '1 minute',
    value: '1m' as AggregateWindowValue,
    seconds: windowToSeconds('1m')
  },
  {
    label: '2 minutes',
    value: '2m' as AggregateWindowValue,
    seconds: windowToSeconds('2m')
  },
  {
    label: '5 minutes',
    value: '5m' as AggregateWindowValue,
    seconds: windowToSeconds('5m')
  },
  {
    label: '10 minutes',
    value: '10m' as AggregateWindowValue,
    seconds: windowToSeconds('10m')
  },
  {
    label: '30 minutes',
    value: '30m' as AggregateWindowValue,
    seconds: windowToSeconds('30m')
  },
  {
    label: '1 hour',
    value: '1h' as AggregateWindowValue,
    seconds: windowToSeconds('1h')
  },
  {
    label: '3 hours',
    value: '3h' as AggregateWindowValue,
    seconds: windowToSeconds('3h')
  },
  {
    label: '6 hours',
    value: '6h' as AggregateWindowValue,
    seconds: windowToSeconds('6h')
  },
  {
    label: '1 day',
    value: '1d' as AggregateWindowValue,
    seconds: windowToSeconds('1d')
  },
  {
    label: '7 days',
    value: '7d' as AggregateWindowValue,
    seconds: windowToSeconds('7d')
  },
  {
    label: '1 month',
    value: '1mo' as AggregateWindowValue,
    seconds: windowToSeconds('1mo')
  }
...
const {
  loading: isExporting,
  startLoading: startExporting,
  endLoading: endExporting
} = useLoading(false); // Separate loading state for export
...
const aggregateWindowOptions = computed<AggregateWindowOption[]>(() => {
  ...
  return allAggregateWindowOptions.map((option) => ({
    ...
    disabled:
      (option.value === 'no_aggregate' && minSeconds > -1) ||
      (option.seconds !== -1 && option.seconds < minSeconds)
  }));
});

const showAggregateFunction = computed(
  () => filterParams.aggregate_window !== 'no_aggregate'
);

// *** ADDED: Computed properties for selected labels ***
const selectedTimeRangeLabel = computed(() => {
  return (
    timeRangeOptions.value.find((opt) => opt.value === filterParams.time_range)
      ?.label ?? 'Select time'
  ); // Fallback text
});
...
const selectedAggregateWindowLabel = computed(() => {
  ...
  return (
    allAggregateWindowOptions.find(
      (opt) => opt.value === filterParams.aggregate_window
...
const selectedAggregateFunctionLabel = computed(() => {
  ...
  return (
    aggregateFunctionOptions.value.find(
      (opt) => opt.value === filterParams.aggregate_function
    )?.label ?? 'Select Method'
  ); // Fallback text
});
...
const fetchData = async (isExport = false) => {
  ...
  try {
    ...
    const response: { data: TimeSeriesItem[] | null; error: any } =
      await telemetryDataHistoryList(params);
    ...
    if (response && response.data && !response.error) {
      if (isExport) {
        ...
        const receivedData: TimeSeriesItem[] = (response.data ||
          []) as TimeSeriesItem[];
      ...
      if (!isExport) {
        ...
        window.$message?.error(
          `Failed to fetch data: ${response?.error?.message || 'Unknown error'}`
        );
      } else {
        window.$message?.error(
...
watch(
  ...
  (newRange) => {
    if (newRange && newRange.length === 2) {
      ...
      if (filterParams.time_range === 'custom') {
        logger.info(
          'Custom date range updated, triggering validation and fetch.'
        );
...
watch(
  ...
  (newFunction) => {
    ...
    if (filterParams.aggregate_window !== 'no_aggregate') {
      logger.info(
        `Aggregate function changed to: ${newFunction}, triggering validation.`
      );
      ...
      logger.warn(
        `Aggregate function changed (${newFunction}) while window is 'no_aggregate'. Ignoring.`
      );
    }
  }
);
...
const validateAndFetch = () => {
  const minSeconds = currentMinWindowSeconds.value;
  const currentWindowSeconds = windowToSeconds(
    filterParams.aggregate_window ?? 'no_aggregate'
  );
  ...
  if (minSeconds > -1 && currentWindowSeconds === -1) {
    const firstValidOption = aggregateWindowOptions.value.find(
      (opt) => !opt.disabled && opt.value !== 'no_aggregate'
    );
    const newWindow = firstValidOption ? firstValidOption.value : '30s'; // Fallback to 30s
    if (filterParams.aggregate_window !== newWindow) {
      logger.info(
        `Aggregate window 'no_aggregate' is invalid. Resetting to '${newWindow}'.`
      );
      filterParams.aggregate_window =
        newWindow as FilterParams['aggregate_window'];
      windowChanged = true;
    }
  }
  // 2. Check if a specific interval is selected but is now too small
    ...
    const newWindow = firstValidOption
  ...
  if (
    filterParams.time_range === 'custom' &&
    (!filterParams.start_time || !filterParams.end_time)
  ) {
    logger.info(
      'Validation complete. Waiting for custom date range selection...'
    );
    ...
    logger.info(
      'Validation complete. Triggering fetchData and emitting filterParams.'
    );
...
const handleExport = () => {
  if (
    ...
    window.$message?.warning(
      'Please select a custom time range before exporting'
    );
    return;
  }
