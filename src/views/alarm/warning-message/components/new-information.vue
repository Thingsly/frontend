  ...
  if (data) {
    setTimeout(() => {
      ...
      const operatorBtns: { btnName: string; type: string; color: string }[] = [
        ...
        {
          btnName: $t('page.manage.common.status.enable'),
          type: 'enable',
          color: 'success'
        },
...
async function editInfos() {
  ...
  if (data) {
    params.enabled === 'Y'
      ? message.success($t('common.startSuccess'))
      : message.success($t('common.stopSuccess'));

    list();
  } else {
    params.enabled === 'Y'
      ? message.error($t('common.startFail'))
      : message.error($t('common.stopFail'));
  }
}
