...
<template>
  <div class="h-full">
    <NCard :title="$t('generate.icon-example')" :bordered="false" class="rounded-8px shadow-sm">
      ...
      <WebSiteLink :label="'iconify' + ' URL' + ': '" link="https://icones.js.org/" class="mt-10px" />
    </NCard>
    <NCard :title="$t('generate.custom-icon-example')" :bordered="false" class="mt-10px rounded-8px shadow-sm">
      <div class="pb-12px text-16px">
        The SVG files located in the src/assets/svg-icon folder can be directly rendered in the template using icon-local-{filename},
        where icon-local is the value of VITE_ICON_LOCAL_PREFIX defined in the .env file
      </div>
      <div class="grid grid-cols-10">
        <div class="mt-5px flex-x-center">
