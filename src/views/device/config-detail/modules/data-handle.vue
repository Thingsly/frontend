...
-- Description: This function is an encoder that converts the input message into a format recognizable by the platform or device.
-- Parameters:
--   msg: The input message, which can be a string of any data type.
-- Returns:
--   The encoded message as a JSON string.
-- Note: To convert between string and JSON object, import the JSON library: local json = require("json")
-- Example: Convert string to JSON object: local jsonData = json.decode(msgString)
-- Example: Convert JSON object to string: local jsonStr = json.encode(jsonTable)

local json = require("json")
local jsonData = json.decode(msg)

-- Example: if jsonData.temp then
--            jsonData.temp = jsonData.temp * 10
--          end
...
const doQuiz = async () => {
  ...
  dataScriptQuiz(configForm.value).then(({ data }: any) => {
    configForm.value.resolt_analog_input = data
...
<template>
  ...
  >
    ...
    >
      <NFormItem
        :class="getPlatform ? 'w-100%' : 'w-50%'"
        :label="$t('page.manage.menu.form.title')"
        path="name"
      >
        <NInput
          v-model:value="configForm.name"
          :placeholder="$t('generate.enter-title')"
        />
      </NFormItem>
      <NFormItem
        :class="getPlatform ? 'w-100%' : 'w-50%'"
        :label="$t('generate.processing-type')"
        path="script_type"
      >
        <NSelect
      ...
      <NFormItem
        class="w-100%"
        :label="$t('device_template.table_header.description')"
        path="description"
      >
        <NInput
      ...
      >
        <NSwitch
          v-model:value="configForm.enable_flag"
          checked-value="Y"
          unchecked-value="N"
        />
      </NFormItem>
      <NFormItem
        class="w-100%"
        :label="$t('generate.simulate-input')"
        path="last_analog_input"
      >
        <NInput
          v-model:value="configForm.last_analog_input"
          type="textarea"
          :rows="2"
        />
      </NFormItem>
      <NFormItem
        class="w-100%"
        :label="$t('generate.debug-run-result')"
        path="resolt_analog_input"
      >
        <NInput
          v-model:value="configForm.resolt_analog_input"
          :rows="5"
          :disabled="true"
          type="textarea"
        />
      ...
      <NFormItem>
        <NButton type="primary" @click="doQuiz">{{
          $t('common.debug')
        }}</NButton>
      </NFormItem>
    </NForm>
