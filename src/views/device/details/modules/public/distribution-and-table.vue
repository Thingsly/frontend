...
const props = defineProps<{
  ...
  submitApi?: (
    params: any
  ) => Promise<FlatResponseSuccessData | FlatResponseFailData>;
  expectApi?: (
    params: any
  ) => Promise<FlatResponseSuccessData | FlatResponseFailData>;
  fetchDataApi: (
...
const submit = async () => {
  try {
    ...
    if (formModel.expected) {
      if (props.expectApi) {
        const expiry = formModel.time
          ? new Date().getTime() + formModel.time * 60 * 60 * 1000
          : null;
    ...
    window.$message?.error(
      $t('common.validateFail') || 'Validation failed, please check your input.'
    );
...
<template>
  <div class="">
    <div class="m-b-20px flex items-center">
      <NButton v-if="buttonName" type="primary" @click="openDialog">{{
        buttonName
      }}</NButton>
    ...
    >
      <NGridItem v-for="item in commandList" :key="item.id">
        <NCard hoverable>
          <div
            class="title cursor-pointer ellipsis-text text-16px font-600"
    ...
    >
      <n-card
        :title="
          isCommand
            ? $t('generate.issueCommand')
            : $t('generate.issue-attribute')
        "
      >
        <NForm
          ref="formRef"
          :model="formModel"
          :rules="rules"
          :label-placement="formModel.expected ? 'left' : 'top'"
        >
          <div v-if="expect" class="flex">
            ...
            <NFormItem
              v-if="formModel.expected"
              :label="$t('generate.expirationTime')"
              class="ml-20px"
            >
              <div class="flex-ai-c flex">
                <n-input-number
          ...
          <div v-else>
            <div v-if="formModel.commandValue !== ''" class="title">
              {{ $t('common.param') }}
            </div>
