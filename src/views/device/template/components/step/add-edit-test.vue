...
const readAndWriteOptions: any = reactive(
  ['R-Read Only', 'RW-Read/Write'].map((v) => ({
    label: v,
    value: v
  }))
);
...
<template>
  ...
  >
    <n-form-item
      :label="$t('device_template.table_header.dataName')"
      path="data_name"
    >
      <n-input
    ...
    <n-form-item
      :label="$t('device_template.table_header.dataIdentifier')"
      path="data_identifier"
    >
      <n-input
        v-model:value.trim="addFrom.data_identifier"
        :placeholder="
          $t('device_template.table_header.pleaseEnterTheDataIdentifier')
        "
      />
    </n-form-item>
    <n-form-item
      :label="$t('device_template.table_header.dataType')"
      path="data_type"
    >
      <n-select
    ...
    <template v-if="addFrom.data_type === 'Enum'">
      <EnumInfo
        :additional-info="addFrom.additional_info"
        @update-additional-info="updateAdditionalInfo"
      />
    </template>
    <n-form-item
      :label="$t('device_template.table_header.readAndWriteSign')"
    ...
    <n-form-item :label="$t('device_template.table_header.unit')">
      <n-input
        v-model:value.trim="addFrom.unit"
