...
const addParameterRules: any = reactive({
  ...
  data_identifier: {
    ...
    trigger: ['blur', 'input'],
    message: $t(
      'device_template.table_header.PleaseEnterTheParameterIdentifier'
    )
  },
  param_type: {
    required: true,
...
const parameterSubmit: () => void = async () => {
  ...
  if (addParameterFrom.param_type === 'Enum') {
    const enum_config = addParameterFrom.enum_config.filter(
      (v) => v.value && v.desc
    );
    if (enum_config.length < 1) {
      window.$message?.error('Please add an enum item! ');
      return;
    }
  ...
  if (addFlag.value) {
    ...
    const index: number = eventsData.findIndex(
      (item) => item.id === addParameterFrom.id
    );
...
<template>
  ...
  >
    ...
    <n-form-item :label="$t('device_template.table_header.commandDescription')">
      <n-input
        ...
        :placeholder="
          $t('device_template.table_header.pleaseEnterACommandDescription')
        "
        type="textarea"
      />
    </n-form-item>
  ...
  >
    ...
    >
      <n-form-item
        :label="$t('device_template.table_header.parameterName')"
        path="data_name"
      >
        <n-input
          v-model:value.trim="addParameterFrom.data_name"
          :placeholder="
            $t('device_template.table_header.PleaseEnterTheParameterName')
          "
        />
      </n-form-item>
      <n-form-item
        :label="$t('device_template.table_header.ParameterIdentifier')"
        path="data_identifier"
      >
        <n-input
          v-model:value.trim="addParameterFrom.data_identifier"
          :placeholder="
            $t('device_template.table_header.PleaseEnterTheParameterIdentifier')
          "
        />
      </n-form-item>
      <n-form-item
        :label="$t('device_template.table_header.ParameterType')"
        path="param_type"
      >
        <div>
          <n-select
            ...
            :placeholder="
              $t('device_template.table_header.PleaseSelectParameterType')
            "
            class="w-150px"
          />
          <template v-if="addParameterFrom.param_type === 'Enum'">
            ...
            <div class="enum-item-list">
              ...
              <div
                v-for="(item, index) in addParameterFrom.enum_config"
                :key="index"
                class="row tr"
              >
                <div class="col value">
                ...
                <div class="col desc">
                  <n-input
                    v-model:value.trim="item.desc"
                    :placeholder="$t('common.input')"
                  />
                </div>
                <div class="col">
                  <NButton type="primary" @click="removeEnumItem(index)">{{
                    $t('common.remove')
                  }}</NButton>
                </div>
              </div>
              <NButton type="primary" @click="addEnumItem">{{
                $t('device_template.addEnumItem')
              }}</NButton>
            </div>
          </template>
        </div>
      ...
      <n-form-item :label="$t('device_template.table_header.description')">
        <n-input
          ...
          :placeholder="
            $t('device_template.table_header.PleaseEnterADescription')
          "
          type="textarea"
        />
      </n-form-item>
    ...
    <div class="box2">
      <NButton class="m-r3" @click="addParameter = false">{{
        $t('generate.cancel')
      }}</NButton>
      <NButton type="primary" @click="parameterSubmit">{{
        $t('device_template.confirm')
      }}</NButton>
    </div>
  </NModal>
