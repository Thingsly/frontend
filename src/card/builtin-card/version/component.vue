    ...
    >
      ...
      <div class="flex flex-col items-center mt-4">
        ...
        <n-tag type="success" size="medium" round class="animate-pulse">
          ...
          {{ isLatestVersion?$t('card.versionInfo.latest'):'Latest Version: '+(latestVersion||'--') }}
        </n-tag>
      </div>
    </div>
  </div>
...
onMounted(async() => {
  try {
    const res = await axios.get('https://api.github.com/repos/Thingsly/backend/tags')
    if(res?.data?.[0]?.name){
      latestVersion.value = res.data[0].name || '--'
    }
  } catch (error) {
    console.warn('Failed to get latest version information:', error)
    latestVersion.value = '--'
  }
