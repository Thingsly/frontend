<template>
  <div class="p-4 h-full bg-white dark:bg-gray-800 rounded-lg shadow-md flex flex-col">
    <div class="flex justify-between items-center mb-3 flex-shrink-0">
      <h3 class="text-base font-semibold text-gray-800 dark:text-gray-100">{{ $t('card.alarmInfo.title', 'Alarm Information') }}</h3>
...
const getStatusInfo = (status: string): { label: string; type: 'default' | 'error' | 'warning' | 'info' | 'success' } => {
  switch (status) {
    case 'H':
...
const columns: DataTableColumns<AlarmData> = [
...

  {
    ...
    render(row) {
      ...
      return h(
        NTag,
        { type: statusInfo.type, size: 'small', round: true },
